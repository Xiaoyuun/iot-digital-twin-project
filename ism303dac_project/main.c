...
int main() {
    ...
    ism303dac_xl_data_rate_set(&accel_ctx, ISM303DAC_XL_ODR_100Hz_HR); // High-resolution 100 Hz
    ism303dac_xl_full_scale_set(&accel_ctx, ISM303DAC_XL_2g); // Set full scale to ±2g

    // Configure magnetometer
    ism303dac_mg_operating_mode_set(&mag_ctx, ISM303DAC_MG_CONTINUOUS_MODE); // Continuous mode
    ism303dac_mg_data_rate_set(&mag_ctx, ISM303DAC_MG_ODR_100Hz); // 100 Hz output data rate

    // Main loop to read and print data
    while (1) {
        // Check if accelerometer data is available
        ism303dac_status_a_t status;
        ism303dac_xl_status_reg_get(&accel_ctx, &status);
        if (status.drdy) {
            int16_t accel_data[3];
            ism303dac_acceleration_raw_get(&accel_ctx, accel_data);
            float accel_mg[3];
            for (int i = 0; i < 3; i++) {
                accel_mg[i] = ism303dac_from_fs2g_to_mg(accel_data[i]);
            }
            printf("Accel [mg]: X=%.2f, Y=%.2f, Z=%.2f\n", accel_mg[0], accel_mg[1], accel_mg[2]);
        }

        // Check if magnetometer data is available
        ism303dac_status_reg_m_t mag_status;
        ism303dac_mg_status_get(&mag_ctx, &mag_status);
        if (mag_status.zyxda) {
            int16_t mag_data[3];
            ism303dac_magnetic_raw_get(&mag_ctx, mag_data);
            float mag_mgauss[3];
            for (int i = 0; i < 3; i++) {
                mag_mgauss[i] = ism303dac_from_lsb_to_mG(mag_data[i]);
            }
